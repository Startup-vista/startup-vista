        ...
        <div className="flex flex-col w-full min-h-screen bg-primary-200">
            ...
            <div className="container mx-auto px-6 -mt-20">
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-1 mt-4 md:mt-0 md:ml-auto max-w-md">
                        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                            <h2 className="font-bold text-xl mb-4">Organization status</h2>

                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    <span className="text-text-800">Established</span>
                                    <span className="text-text-800 font-medium">{formatFirestoreDateAdmin(userData.establishedDate)}</span>
                                </div>
                                ...
                                <div className="flex justify-between">
                                    <span className="text-text-800">Team Size</span>
                                    <span className="text-text-800 font-medium">{userData.teamSize}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="flex-1 md:mt-0 md:ml-auto max-w-md">
                        <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                            <h2 className="font-bold text-xl mb-4">Account Holder Details</h2>

                            <div className="space-y-3">
                                <div className="flex justify-between">
                                    <span className="text-text-800">Name</span>
                                    <span className="text-text-800 font-medium">{userData.personalName}</span>
                                </div>

                                <div className="flex justify-between">
                                    <span className="text-text-800">Designation</span>
                                    <span className="text-text-800 font-medium">{userData.designation}</span>
                                </div>

                                <div className="flex justify-between">
                                    <span className="text-text-800">Email</span>
                                    <span className="text-text-800 font-medium">{userData.personalEmail}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-text-800">Phone</span>
                                    <span className="text-text-800 font-medium">{userData?.personalPhone || "Not Available"}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-text-800">Date of Birth</span>
                                    <span className="text-text-800 font-medium">{formatFirestoreDateAdmin(userData.birthDate)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-text-800">Gender</span>
                                    <span className="text-text-800 font-medium">{userData.gender}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            {userData.fundingEntries.map((card, index) => (
                                <div key={index} className="bg-white p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div className="flex items-center mt-2">
                                        ...
                                        <span className="text-sm text-gray-500 ml-2">{formatFirestoreDateAdmin(card.fundingDate)}</span>
                                    </div>

                                    <div className="flex justify-between items-center mt-4">
                                        <div>
                                            <div className="text-2xl font-bold">{card.fundingAmount}</div>
                ...
                <div className="flex flex-col md:flex-row gap-6 my-10">
                    <div className="flex flex-col items-start">
                        <h2 className="font-bold text-2xl mb-4">Registered Company Name</h2>
                        <h3 className="text-text-800 text-xl font-bold">{userData?.companyName}</h3>
                    </div>
                    <div className="flex flex-col items-start">
                        <h2 className="font-bold text-2xl mb-4">Incorporation Certificate</h2>
                        <Link href={userData?.incorporationCertificate} className="flex items-center gap-2 px-4 py-2 rounded-md border border-text-800 text-text-800 bg-white">
                            <ExternalLink size={18} />
                            <span>View Incorporation Certificate</span>
                        </Link>
                    </div>
                </div>
                )}
